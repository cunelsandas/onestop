...
if ($_POST['action'] == "save") {
	...
	$des_file=$_SERVER['DOCUMENT_ROOT']."/onestop/fileupload/webcam/".$newfilename;
	//Create the image

	$fp = fopen($des_file, 'w');
	fwrite($fp, $unencoded);
	fclose($fp);
	$filename="INSERT INTO filename(tablename,masterid,filename) Values('".$tablename."','".$lid."','".$newfilename."')";
	...
	$sql = "SELECT * FROM ".$tablename." INNER JOIN filename ON ".$tablename.".id = filename.masterid";
	$rs = rsQuery($sql);
	$data = array();
