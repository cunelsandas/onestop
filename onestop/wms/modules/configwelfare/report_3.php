...
if (isset($_POST['btsave']) != "") {
	...
	if ($moo != "") {
		...
		$sql1 = "SELECT b.type as type,a.*,b.* FROM tb_citizen a
		INNER JOIN tb_welfare_request b on a.personid = b.personid
		WHERE a.moo = '$moo' AND b.confirmdate < '$year_str-12-31'";

		$sql2 = "SELECT b.type as type,a.*,b.* FROM tb_citizen a
		INNER JOIN tb_welfare_request b on a.personid = b.personid
		WHERE a.moo = '$moo' AND b.confirmdate < '$year_end-12-31'";
	}
			...
			<table width="100%">
				<tr>
					<td align="center">ปี พ.ศ.
						&nbsp;<select name="frm_year_str" class="form-control" style="width:100px;">
						<?php
						$year = date('Y')+543;
						$numyear = $year-10;
						...
						&nbsp;ถึง&nbsp;<select name="frm_year_end" class="form-control" style="width:100px">
						<?php
						$year = date('Y')+543;
						$numyear = $year-10;
						for ($i=$numyear; $i <= $year; $i++) {
...
<div name="data_table" id="data_table">
    ...
    <tr>
			<th></th>
			<th colspan="3" align="center">ประเภทเบี้ย</th>
			<th colspan="1"></th>
		</tr>
		<tr>
			<th>ปี</th>
