...
$code=trim($_GET['id']);
$result=rsQuery("Select * from tb_infrastructure Where id=$code");
if (!$result) {
    ...
    $receiveno = FindRS("SELECT * FROM tb_request WHERE table_name='tb_infrastructure' AND master_id = '$code'",receiveno);
    $datein = FindRS("SELECT * FROM tb_request WHERE table_name='tb_infrastructure' AND master_id = '$code'",datein);
...
//------------------------------------------------------------------
    ...
    $detail= "ข้าพเจ้า ".$name." อยู่".$addrass." หมายเลขโทรศัพท์ ".$row['tel']." &nbsp;อีเมล ".$row['email']." หมายเลขไอพี ".$ip;
...
echo '<div class="page">
							...
							<table height="100%" width="100%" border="0">
								...
								<caption style="padding: 20px"><h3> คำร้องขอซ่อมแซมสาธราณูปโภค </h3></caption>
								<tr>
                ...
                <tr>
                  <td colspan="2" style="text-align:justify">
                  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                  มีความประสงค์ให้ '.$customer_name.' ตรวจสอบและซ่อมแซมสาธารณูปโภค หมู่ที่ '.$row['moo2'].' ตำบล '.$customer_tambon.' อำเภอ '.$customer_amphur.' จังหวัด '.$customer_province.' ดังรายละเอียดต่อไปนี้
                  </td>
                </tr>
...
<?php
$sqls  =  "select * from files_image where table_name='tb_infrastructure' and master_id='$code' ";
$rss = rsQuery($sqls);
...
?>
        ...
        <tr><td align="center"><h2 style="margin-bottom:0px">รูปภาพประกอบ</h2>
                        ...
                        src='../../../fileupload/infrastructure/".$rows['file_name']."'>";
                            $count++;
                        }
                        ?>
        </td></tr>
